...
export class SQLDatabaseManager {
  ...
  constructor(dbType: DatabaseType = DatabaseType.SQLITE, dbUrl: string = ":memory:") {
    this.dbType = dbType;
    ...
    this.init().catch(err => console.error("Failed to initialize database:", err));
  }
  ...
  private async initSqlite(): Promise<void> {
    ...
    await this.runSqlite("CREATE INDEX IF NOT EXISTS idx_history_memory_id ON history(memory_id)");
    await this.runSqlite("CREATE INDEX IF NOT EXISTS idx_history_updated_at ON history(updated_at)");
  }
  ...
  private async initPostgres(): Promise<void> {
    ...
    await this.pgPool.query("CREATE INDEX IF NOT EXISTS idx_history_memory_id ON history(memory_id)");
    await this.pgPool.query("CREATE INDEX IF NOT EXISTS idx_history_updated_at ON history(updated_at)");
  }
  ...
  private async initMysql(): Promise<void> {
    ...
    try {
      await this.mysqlPool.query("CREATE INDEX idx_history_memory_id ON history(memory_id)");
    } catch (e) {
      // Index might already exist
    }
    
    try {
      await this.mysqlPool.query("CREATE INDEX idx_history_updated_at ON history(updated_at)");
    } catch (e) {
      // Index might already exist
    }
  }
  ...
  private async runSqlite(sql: string, params: any[] = []): Promise<void> {
    return new Promise((resolve, reject) => {
      if (!this.sqliteDb) return reject(new Error("SQLite database not initialized"));
  ...
  async addHistory(
    ...
    switch (this.dbType) {
      ...
      case DatabaseType.POSTGRESQL:
        if (!this.pgPool) throw new Error("PostgreSQL database not initialized");
